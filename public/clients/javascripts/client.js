...
class Hp {
    ...
    async setNotification() {
        ...
        const get_activity_api_url = base_url("api/get_activity/" + day);
            ...
            if(data.length > 0) {
                data.forEach((value) => {
                    ...
                    if (currentTime > adjustedTargetTime) {
                        ...
                        this.setDynamicIsland("assets/images/" + value.image, value.text_its_time);
                    }
                });
            }
        })
        ...
        .finally(() => {
            ...
            this.notifications.forEach((value) => {
                hp_notification.appendChild(this.createNotificationElement(value.id, "assets/images/" + value.image, value.title, this.formatTime(value.date), value.text));
            });
        });
    }
...
class Tablet {
    ...
    constructor() {
        ...
        tablet_videos.addEventListener("ended", () => {
            ...
            tablet_video.setAttribute("src", "assets/videos/" + this.videos[this.currentVideoIndex].video);
            tablet_videos.load();
            tablet_videos.play();
        });
    }
    ...
    setQrCode() {
        ...
        .then((response) => {
            ...
            if(data["image"] != undefined && data["description"] != undefined) {
                this.qrcode = new QrCodeModel(`assets/qrcodes/${data["image"]}`, data["description"]);
            }
        })
        .catch((error) => {
